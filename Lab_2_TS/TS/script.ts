class Shoes {
    ...
    public set discountPrice(discount: number) {
        this.discount = discount;
        this.discountPrice = this.price - (this.price * (this.discount / 100));
    }

    constructor(type: string, id: number, size: number, color: string, price: number, discount: number) {
        this.type = type;
        this.id = id;
        this.size = size;
        this.color = color;
...
class Products implements Iterator < Shoes > {
    private pointer = 0;
    ...
    public next(): IteratorResult < Shoes > {
    ...
    [Symbol.iterator](): IterableIterator < Shoes > {
        return this;
    }
}
